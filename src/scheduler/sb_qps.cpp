...
namespace saber {
saber::SB_QPS::SB_QPS(const std::string &name,
                      int num_inputs,
                      int num_outputs,
                      int frame_size,
                      unsigned iterations,
                      const AcceptPolicy &ap,
    ...
    _iterations(iterations), _accept_policy(ap), _schedules_pre(frame_size, std::vector<int>(num_inputs, -1)) {
...
void SB_QPS::handle_departures(const std::vector<std::pair<int, int>> &dep_pre) {
...
void SB_QPS::qps(const saber::IQSwitch *sw, size_t frame_id) {
  ...
  switch (_accept_policy) {
    case AcceptPolicy::FFA:allocation = std::unique_ptr<ffa>(new ffa(_match_flag_in, _match_flag_out));
      break;
    case AcceptPolicy::MFA:allocation = std::unique_ptr<mfa>(new mfa(_match_flag_in, _match_flag_out));
      break;
    case AcceptPolicy::MWFA:
      allocation = std::unique_ptr<mwfa>(new mwfa(_match_flag_in, _match_flag_out, _cf_packets_counter));
      break;
  }
  ...
  for (unsigned it = 0; it < _iterations; ++it) {
    ...
    for (unsigned out = 0; out < _num_outputs; ++out) {
      ...
      if (proposals[out].size() > 1) {
        ...
        std::sort(proposals[out].begin(), proposals[out].end(), [out, this](unsigned lhs, unsigned rhs) {
          return (this->_cf_packets_counter[lhs][out] > this->_cf_packets_counter[rhs][out]);
        });
      }
...
void SB_QPS::schedule(const saber::IQSwitch *sw) {
  ...
  std::copy(_schedules_pre[frame_id].begin(), _schedules_pre[frame_id].end(), _in_match.begin());
  std::fill(_schedules_pre[frame_id].begin(), _schedules_pre[frame_id].end(), -1);
  qps(sw, frame_id);
