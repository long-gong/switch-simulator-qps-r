...
FairFrame::FairFrame(const std::string &name,
                     int num_inputs,
                     int num_outputs,
                     int frame_size) :
    ...
    _row_sums(num_inputs, 0), _col_sums(num_outputs, 0), _w(0) {
  _cf_rel_time = 0;
...
void FairFrame::handle_arrivals(const IQSwitch *sw) {
  ...
  for (const auto &sd : arrivals) {
    ...
    if (_row_sums[sd.first] < frame_size() && _col_sums[sd.second] < frame_size()) {
      ++_row_sums[sd.first];
      ++_col_sums[sd.second];
...
void FairFrame::schedule(const saber::IQSwitch *sw) {
  ...
  if (frame_id == _frame_size - 1) {
    ...
    auto doubly_stochastic_mat = DenseMatrix<unsigned>{std::move(_cf_packets_counter)};
    padding_to_doubly_stochastic<DenseMatrix<unsigned>>(doubly_stochastic_mat, _w, _row_sums, _col_sums);
    // NOTICE: will change size of _schedules
//    printf("%lu: w = %d\n", _cf_rel_time, _w);
    birkhoff_von_neumann_decomposition<DenseMatrix<unsigned>>(doubly_stochastic_mat, _schedules, _w);
    ...
    _cf_packets_counter.resize(num_inputs(), std::vector<unsigned>(num_outputs(), 0));
...
void FairFrame::display(std::ostream &os) const {
