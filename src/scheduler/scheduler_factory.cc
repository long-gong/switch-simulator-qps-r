...
Scheduler *SchedulerFactory::Create(const json &conf) {
  ...
  if (name == "dummy") {
    ...
    if (conf.count("initial_matching"))
      initial_match = conf["initial_matching"].get<std::vector<int>>();
    ...
    int iteration =
        (conf.count("iterations") ? conf["iterations"].get<int>() : 1);
    ...
    //    int frame_size =
    //    (conf.count("frame_size")?conf["frame_size"].get<int>():(10 *
    //    num_inputs * num_outputs)); unsigned seed = (conf.count("seed") ?
    //    conf["seed"].get<unsigned>()
    //                                        : static_cast<unsigned
    //                                        >(sys_clock_t::now().time_since_epoch().count()));
    //    sched = new RandomizedEdgeColoringCAggarwal(name, num_inputs,
    //    num_outputs, frame_size, seed);
  } else if (name == "sb_qps") {
    int frame_size =
        (conf.count("frame_size") ? conf["frame_size"].get<int>() : 16);
    unsigned seed = (conf.count("seed")
                         ? conf["seed"].get<unsigned>()
                         : static_cast<unsigned>(
                               sys_clock_t::now().time_since_epoch().count()));
    ...
    sched = new SB_QPS(name, num_inputs, num_outputs, frame_size, iterations,
                       accept_policy, seed);
